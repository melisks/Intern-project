...
if ($_SESSION['login_state'] == true && $_SESSION['utype'] == "vendor") {
  ...
  echo "<title>Manage My Orders</title>";
  include "../includes/vmenubar.html";
  echo "<script>uname= document.getElementById(\"username\");
  uname.innerHTML = \"$uname\";</script>";
  echo "<div class=\"float-left\"><h1 class\"float-left\">Orders Pending</h1></div>";
  include "../includes/sqlcon.php";
  $userid = $_SESSION['uid'];
  $cinfo = $_GET['cinfo'];
  $total = 0;
  $status = "SELECT * FROM order_info JOIN product_info ON order_info.pid = product_info.pid WHERE vid = '$userid'";
  ...
  while ($rcnt = mysqli_fetch_assoc($result)) {
    ...
    echo "<div class=\"card\" style=\"margin:0px 9px 5px 9px;width: 18rem; height=304.3px\">
        ...
        <div class=\"card-body\" style=\"height=171.433px\">
          ...
          <div>
          <a href=\"itemsent.php?uid=$uid&pid=$pid\" id=\"addtocart\" class=\"btn btn-success\">Item Sent</a>
          <a href=\"ordermanage.php?cinfo=yes\" id=\"buy\" class=\"btn btn-primary\">Customer Info</a>
        </div>
        </div>
      </div>";
  }
