...
if ($_SESSION['login_state'] == true) {
    ...
    echo "<style>#customer{color: white;}.float-left{float:left;}.float-right{float:right;}.bt{width:auto; margin:5px 4px 0px 0px}#productlist{position: absolute; top: 150; z-index:0;}.card-text{height:48px}img{weight:287px;height:192px;}</style>";
    include "../includes/amenubar.html";
    echo "<script>uname= document.getElementById(\"username\");
    uname.innerHTML = \"$uname\";</script>";

    echo "<div class=\"float-left\"><h1 class\"float-left\">All Customers</h1></div>";


    include "../includes/sqlcon.php";
    $status = "SELECT * FROM user_info WHERE utype='customer'";
    ...
    while ($rcnt = mysqli_fetch_assoc($result)) {
        ...
        $products = "SELECT * FROM order_info WHERE uid=$uid";
        ...
        <div class=\"card-body\" style=\"height=171.433px\">
          ...
          <span ><h5 class=\"d-inline\">No. of purchaes $row_count3</h5></span>
          <div>
          <a href=\"deleteaccount.php?uid=$uid&utype=customer\" id=\"delete\" class=\"btn btn-danger\">Delete</a>
          </div>
          </div>
          </div>";
    }
